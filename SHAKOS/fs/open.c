 ...
 *   - do_lseek()
 *   - create_file()
 * @author original: Forrest Yu 2007
 * @date   original: 2008
 *****************************************************************************
 *****************************************************************************/
...
PUBLIC int do_open()
{
	...
	if (flags & O_CREAT) 
	{
		...
		{
			int isdir = 0;
			if (pathname[0] == '$') isdir = 1;
			pin = create_file(pathname, flags);
			if (pin && isdir == 1) put_inode(pin);
		}
	}
	else 
	{
 ...
 *****************************************************************************/
/**
 * List files and directories based current directory
 * 
 * @return number of the files and directories searched
 *****************************************************************************/
PUBLIC int do_ls()
{
	char pathname[MAX_PATH];
	...
	return get_list(dir_inode, I_DIRECTORY, 1) + get_list(dir_inode, I_REGULAR, 1);
...
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect, u32 mode)
{
	...
	new_inode->i_mode = mode; //I_DIRECTORY or I_REGULAR
	new_inode->i_size = 0;
