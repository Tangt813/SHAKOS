 ...
 * @date   original: Tue Jun  3 17:05:10 2008
 *****************************************************************************
 *****************************************************************************/
 ...
 *
 * @note Result will be saved in array abname
 * 
...
PUBLIC void get_Absolute(char *abname, char *path, char *file)
{
	int i = 0, j = 0;
	while (path[i] != 0) // 写入路径
	{
...
PUBLIC int do_unlink()
{
	...
	{
		if (get_list(pin, I_DIRECTORY, 0) + get_list(pin, I_REGULAR, 0) > 0)
		{
			printl("Remove fail: %s is not an empty directory!\n", filename);
			return -3;
		}
		...
		for (i = 0; i < nr_dir_blks; i++)
		{
			...
			for (j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++, pde++)
			{
				if (pde->inode_nr == 0)
					continue;
				char *absopath[512];
				get_Absolute(absopath, pathname, pde->name);
				printl("absopath: %s\n", absopath);
				do_dir_unlink(absopath);
				if (++m >= nr_dir_entries)
					break;
			}
...
PUBLIC int do_dir_unlink(pathname)
{
	if (strcmp(pathname, "/") == 0)
	{
